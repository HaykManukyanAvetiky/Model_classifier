...
class TempletLayer(nn.Module):
  ...
  def __init__(self, n_input, n_output, kernel):
    super(TempletLayer, self).__init__()
    self.conv = nn.Conv2d(n_input, n_output, kernel_size=kernel, stride=(1, 1), bias=False)
    self.bn =  nn.BatchNorm2d(n_output, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
    self.relu = nn.ReLU(inplace=True)
    self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2,  dilation=1, ceil_mode=False)
...
class Net(nn.Module):
  ...
  def __init__(self):
    ...
    self.features = nn.Sequential(
                    OrderedDict([('layer1', TempletLayer( n_input=3, n_output=512, kernel=(3,3)) ),
                                ('layer2', TempletLayer( n_input=512, n_output=256, kernel=(3,3)) ),
                                ('layer3', TempletLayer( n_input=256, n_output=128, kernel=(3,3)) )
                                ]))
    self.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1)) 
    self.classifier = nn.Sequential(
                        OrderedDict([('fc1', nn.Linear(128, 64)),
                                  ('relu1', nn.ReLU()),
